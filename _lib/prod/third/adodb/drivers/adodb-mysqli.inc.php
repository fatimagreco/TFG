...
class ADODB_mysqli extends ADOConnection {
    ...
    var $metaIndexsSQL = "SHOW INDEX FROM `%s`";
	...
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename, $arrExtraArgs=array(), $charset="")
	  {
		...
		*/
		...
		if ($this->persistent && PHP_VERSION > 5.2 && strncmp($argHostname,'p:',2) != 0) $argHostname = 'p:'.$argHostname;
	...
	function MetaIndexes ($table, $primary = FALSE, $owner = false)
	{
		...
		// get index details
        $rs = $this->Execute(sprintf($this->metaIndexsSQL,$table));
