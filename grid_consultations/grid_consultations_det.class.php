...
class grid_consultations_det
{
 ...
 function monta_det()
 {
    ...
    { 
        ...
        $this->consul_date_2 = (isset($Busca_temp['consul_date_input_2'])) ? $Busca_temp['consul_date_input_2'] : ""; 
    } 
    else 
    { 
        $this->consul_date_2 = ""; 
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['sub_dir'] = array(); 
   ...
   { 
       $nmgp_select = "SELECT con.consul_id as consul_id, con.pati_id as pati_id, con.user_id as user_id, str_replace (convert(char(10),con.consul_Date,102), '.', '-') + ' ' + convert(char(8),con.consul_Date,20), str_replace (convert(char(10),con.consul_strtime,102), '.', '-') + ' ' + convert(char(8),con.consul_strtime,20), str_replace (convert(char(10),con.consul_endtime,102), '.', '-') + ' ' + convert(char(8),con.consul_endtime,20), con.consil_sesnum as consil_sesnum, con.consul_reason as consul_reason, con.consul_anot as consul_anot, con.consul_state as consul_state, u.user_docnum as user_docnum, CONCAT(u.user_lname,\",\",u.user_name) as uname, p.pati_docnum as p_pati_docnum, CONCAT(p.pati_lname,\", \",p.pati_name) as pname from " . $this->Ini->nm_tabela; 
   } 
   else 
   { 
       $nmgp_select = "SELECT con.consul_id as consul_id, con.pati_id as pati_id, con.user_id as user_id, con.consul_Date as consul_date, con.consul_strtime as consul_strtime, con.consul_endtime as consul_endtime, con.consil_sesnum as consil_sesnum, con.consul_reason as consul_reason, con.consul_anot as consul_anot, con.consul_state as consul_state, u.user_docnum as user_docnum, CONCAT(u.user_lname,\",\",u.user_name) as uname, p.pati_docnum as p_pati_docnum, CONCAT(p.pati_lname,\", \",p.pati_name) as pname from " . $this->Ini->nm_tabela; 
   } 
   $parms_det = explode("*PDet*", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['chave_det']) ; 
   ...
   {
       $nmgp_select .= " where  con.consul_id = $parms_det[0]" ;  
   } 
   else 
   { 
       $nmgp_select .= " where  con.consul_id = $parms_det[0]" ;  
   ...
   { 
       $this->consul_reason = $this->Db->BlobDecode($rs->fields[7]) ;  
   } 
   else
   { 
   ...
   if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_ibase))
   { 
       $this->consul_anot = $this->Db->BlobDecode($rs->fields[8]) ;  
   } 
   else
   { 
   ...
   $this->user_docnum = $rs->fields[10] ;  
   $this->uname = $rs->fields[11] ;  
   $this->p_pati_docnum = $rs->fields[12] ;  
   $this->p_pati_docnum = (string)$this->p_pati_docnum;
   $this->pname = $rs->fields[13] ;  
...
$nm_saida->saida("</script>\r\n");
   ...
   if (($this->Ini->sc_export_ajax || $this->Ini->Export_det_zip) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['det_print'] == "print")
   {
       ...
       $NM_css_cmp  = $this->Ini->path_link . "grid_consultations/grid_consultations_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css";
       $nm_saida->saida("  <style type=\"text/css\">\r\n");
   ...
   elseif (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
       ...
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_consultations/grid_consultations_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
   }
   else
   {
   ...
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("</HEAD>\r\n");
    $removeMargin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['remove_margin'] ? 'margin: 0;' : '';
    $removeBorder = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['remove_border'] ? 'border-width: 0' : '';
   if (!$this->Ini->Export_html_zip && !$this->Ini->Export_det_zip && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['det_print'] == "print")
   {
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
   ...
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['cmp_acum']);
   ...
   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['compact_mode']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['link_info']['compact_mode']) {
   ...
   $SC_Label = (isset($this->New_label['consul_id'])) ? $this->New_label['consul_id'] : "Consul Id"; 
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_id))); 
   $conteudo_original = $conteudo; 
          ...
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_id', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_id', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_id_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_consul_id_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_id\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_date))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_date', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_date', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_date_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_consul_date_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_date\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_strtime))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_strtime', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_strtime', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_strtime_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_consul_strtime_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_strtime\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $SC_Label = (isset($this->New_label['consul_endtime'])) ? $this->New_label['consul_endtime'] : "Consul Endtime"; 
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_endtime))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_endtime', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_endtime', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_endtime_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_consul_endtime_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_endtime\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $SC_Label = (isset($this->New_label['consil_sesnum'])) ? $this->New_label['consil_sesnum'] : "Consil Sesnum"; 
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consil_sesnum))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consil_sesnum', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consil_sesnum', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consil_sesnum_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_consil_sesnum_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consil_sesnum\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_reason)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->consul_reason)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_reason', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_reason', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_reason_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_consul_reason_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_reason\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $SC_Label = (isset($this->New_label['consul_anot'])) ? $this->New_label['consul_anot'] : "Consul Anot"; 
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_anot)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->consul_anot)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_anot', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_anot', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_anot_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_consul_anot_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_anot\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $SC_Label = (isset($this->New_label['consul_state'])) ? $this->New_label['consul_state'] : "Consul State"; 
   $conteudo = trim(NM_encode_input(sc_strip_script($this->consul_state))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'consul_state', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'consul_state', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_consul_state_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_consul_state_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_consul_state\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       ...
       $conteudo = trim(sc_strip_script($this->user_docnum)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->uname)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->uname)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'uname', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'uname', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_uname_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_uname_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_uname\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   $conteudo = trim(NM_encode_input(sc_strip_script($this->p_pati_docnum))); 
   $conteudo_original = $conteudo; 
          if ($conteudo === "") 
          { 
              $conteudo = "&nbsp;" ; 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'p_pati_docnum', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'p_pati_docnum', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_p_pati_docnum_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldOddVert css_p_pati_docnum_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_p_pati_docnum\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("  <TR class=\"scGridLabel\">\r\n");
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf")
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->pname)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->pname)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'pname', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'pname', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_pname_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_pname_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_pname\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("</TABLE>\r\n");
   if(isset($_SESSION['scriptcase']['proc_mobile']) && $_SESSION['scriptcase']['proc_mobile'])
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"grid_consultations_doc.php?nmgp_parms=\" + campo1, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   {
      global $nm_saida;
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_consultations']['pdf_det']) 
   { 
       ...
       if ($this->nmgp_botoes['det_pdf'] == "on")
       {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_consultations/grid_consultations_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=es&conf_socor=N&sc_ver_93=s&password=n&pdf_zip=N&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $this->nm_btn_exist['det_print'][] = "Dprint_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_consultations/grid_consultations_config_print.php?nm_opc=detalhe&nm_cor=CO&password=n&language=es&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
