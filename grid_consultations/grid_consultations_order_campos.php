...
<?php
   ...
   $Ord_Cmp = new grid_consultations_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_consultations_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select_orig'] = array();
   }
   ...
   if ($use_alias == "S")
   {
       $tab_def_campos['consul_date'] = "consul_date";
   }
   else
   {
       $tab_def_campos['consul_date'] = "con.consul_Date";
   }
   $tab_labels["consul_date"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_date"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_date"] : "Fecha";
   $tab_ger_campos['consul_strtime'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['consul_strtime'] = "consul_strtime";
   }
   else
   {
       $tab_def_campos['consul_strtime'] = "con.consul_strtime";
   }
   $tab_labels["consul_strtime"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_strtime"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_strtime"] : "Hora";
   $tab_ger_campos['p_pati_docnum'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['p_pati_docnum'] = "p_pati_docnum";
   }
   else
   {
       $tab_def_campos['p_pati_docnum'] = "p.pati_docnum";
   }
   $tab_labels["p_pati_docnum"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["p_pati_docnum"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["p_pati_docnum"] : "Nro. Documento";
   $tab_ger_campos['pname'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['pname'] = "pname";
   }
   else
   {
       $tab_def_campos['pname'] = "CONCAT(p.pati_lname,\", \",p.pati_name)";
   }
   $tab_labels["pname"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["pname"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["pname"] : "Paciente";
   $tab_ger_campos['consul_reason'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['consul_reason'] = "consul_reason";
   }
   else
   {
       $tab_def_campos['consul_reason'] = "con.consul_reason";
   }
   $tab_labels["consul_reason"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_reason"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["consul_reason"] : "Motivo";
   $tab_ger_campos['uname'] = "on";
   if ($use_alias == "S")
   {
       $tab_def_campos['uname'] = "uname";
   }
   else
   {
       $tab_def_campos['uname'] = "CONCAT(u.user_lname,\",\",u.user_name)";
   }
   $tab_labels["uname"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["uname"])) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['labels']["uname"] : "Encargado";
   $tab_ger_campos['consul_reason'] = "none";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_consultations']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_consultations']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_consultations_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_consultations']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
