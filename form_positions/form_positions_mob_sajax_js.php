  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_form_positions_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_form_positions_mob_form();
	}
...
?>
  ...
  // ---------- Validate pos_name
  function do_ajax_form_positions_mob_validate_pos_name()
  {
    var nomeCampo_pos_name = "pos_name";
    var var_pos_name = scAjaxGetFieldText(nomeCampo_pos_name);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_form_positions_mob_validate_pos_name(var_pos_name, var_script_case_init, do_ajax_form_positions_mob_validate_pos_name_cb);
  } // do_ajax_form_positions_mob_validate_pos_name

  function do_ajax_form_positions_mob_validate_pos_name_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "pos_name";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_form_positions_mob_validate_pos_name_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_form_positions_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_form_positions_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_pos_name = scAjaxGetFieldText("pos_name");
    var var_pos_id = scAjaxGetFieldText("pos_id");
    ...
    scAjaxProcOn();
    x_ajax_form_positions_mob_submit_form(var_pos_name, var_pos_id, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_form_positions_mob_submit_form_cb);
  } // do_ajax_form_positions_mob_submit_form

  function do_ajax_form_positions_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("table");
      scAjaxHideErrorDisplay("pos_name");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_form_positions_mob_submit_form_cb_after_alert);
  } // do_ajax_form_positions_mob_submit_form_cb
  function do_ajax_form_positions_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_form_positions_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_form_positions_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_form_positions_mob_navigate_form, function() {});
    } else {
      perform_ajax_form_positions_mob_navigate_form();
    }
  }

  function perform_ajax_form_positions_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxHideErrorDisplay("table");
    scAjaxHideErrorDisplay("pos_name");
    var var_pos_id = document.F2.pos_id.value;
    ...
    scAjaxProcOn();
    x_ajax_form_positions_mob_navigate_form(var_pos_id, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_form_positions_mob_navigate_form_cb);
  } // do_ajax_form_positions_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_positions_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_form_positions_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetFields(false);
    scAjaxSetVariables();
    document.F2.pos_id.value = scAjaxGetKeyValue("pos_id");
    scAjaxSetSummary();
    scAjaxShowDebug();
    scAjaxSetLabel(true);
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_form_positions_mob_navigate_form_cb_after_alert);
  } // do_ajax_form_positions_mob_navigate_form_cb
  function do_ajax_form_positions_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_form_positions_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_form_positions_mob_navigate_form_cb_after_alert

  function sc_hide_form_positions_mob_form()
  {
  ...
  ajax_field_list[0] = "pos_name";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "pos_name": {"label": "Ocupación", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["pos_name"][1] = "pos_name";

  var ajax_field_id = {
    "pos_name": new Array("hidden_field_label_pos_name", "hidden_field_data_pos_name")
  };
