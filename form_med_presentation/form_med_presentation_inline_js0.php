<form name="F2" method=post 
               ...
               target="_self"> 
<input type="hidden" name="mpres_id_" value="<?php echo $this->form_encode_input($this->nmgp_dados_form['mpres_id_']); ?>">
...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_med_presentation_inline']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_med_presentation_inline']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_med_presentation_inline"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_med_presentation']['where_detal']))
  {
?>  
  if (parent && parent.nmsc_iframe_liga_form_med_presentation.scInlineFormReceive)
  {
    parent.nmsc_iframe_liga_form_med_presentation.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_med_presentation_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
?>  
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_med_presentation_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
function lookup_mpres_exits_(campo)
{
   dados  = document.F1.mpres_id_.value;
   jsrsExecute("<?php  echo $this->Ini->path_link . SC_dir_app_name('form_med_presentation_inline')?>/form_med_presentation_inline.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nmgp_opcao=lookup", recebe_mpres_exits_, "mpres_exits_", dados, false);
}
function recebe_mpres_exits_(string)
{
   document.getElementById('id_lookup_mpres_exits_').innerHTML = string;
}
<?php 
