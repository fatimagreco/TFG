...
class grid_gerens_det
{
 ...
 function monta_det()
 {
    ...
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['seq_dir'] = 0; 
    $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['sub_dir'] = array(); 
   ...
   $parms_det = explode("*PDet*", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['chave_det']) ; 
   ...
   {
       $nmgp_select .= " where  gerens_id = $parms_det[0]" ;  
   } 
   else 
   { 
       $nmgp_select .= " where  gerens_id = $parms_det[0]" ;  
...
$nm_saida->saida("</script>\r\n");
   ...
   if (($this->Ini->sc_export_ajax || $this->Ini->Export_det_zip) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['det_print'] == "print")
   {
       ...
       $NM_css_cmp  = $this->Ini->path_link . "grid_gerens/grid_gerens_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css";
       $nm_saida->saida("  <style type=\"text/css\">\r\n");
   ...
   elseif (($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['det_print'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
   {
       ...
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "grid_gerens/grid_gerens_det_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
   }
   else
   {
   ...
   if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['pdf_det'] && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['det_print'] != "print")
   {
   ...
   $nm_saida->saida("</HEAD>\r\n");
    $removeMargin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['remove_margin'] ? 'margin: 0;' : '';
    $removeBorder = isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['remove_border']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['remove_border'] ? 'border-width: 0' : '';
   if (!$this->Ini->Export_html_zip && !$this->Ini->Export_det_zip && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['det_print'] == "print")
   {
       $nm_saida->saida(" <link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/buttons/" . $this->Ini->Str_btn_css . "\" /> \r\n");
   ...
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['cmp_acum']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['cmp_acum']))
   {
       $parms_acum = explode(";", $_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['cmp_acum']);
   ...
   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['compact_mode']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['link_info']['compact_mode']) {
   ...
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
       $conteudo = trim(NM_encode_input(sc_strip_script($this->gerens_name)));
       $conteudo_original =  NM_encode_input(sc_strip_script($conteudo)); 
   }
   else {
       $conteudo = trim(sc_strip_script($this->gerens_name)); 
       $conteudo_original = $conteudo; 
   }
          if ($conteudo === "") 
          { 
          ...
          if(!empty($str_tem_display) && $str_tem_display != '&nbsp;' && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['proc_pdf'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['embutida'] && !empty($conteudo)) 
          { 
              $str_tem_display = $this->getFieldHighlight('quicksearch', 'gerens_name', $str_tem_display); 
              $str_tem_display = $this->getFieldHighlight('advanced_search', 'gerens_name', $str_tem_display); 
          } 
              $conteudo = $str_tem_display; 
   $nm_saida->saida("    <TD class=\"scGridLabelFont css_gerens_name_det_label\"  >" . nl2br($SC_Label) . "</TD>\r\n");
   $nm_saida->saida("    <TD class=\"scGridFieldEvenVert css_gerens_name_det_line\"  " . $this->SC_nowrap . " ALIGN=\"\" VALIGN=\"\"><span id=\"id_sc_field_gerens_name\">" . $conteudo . "</span></TD>\r\n");
   $nm_saida->saida("   \r\n");
   $nm_saida->saida("  </TR>\r\n");
   $nm_saida->saida("</TABLE>\r\n");
   if(isset($_SESSION['scriptcase']['proc_mobile']) && $_SESSION['scriptcase']['proc_mobile'])
...
//--- 
   ...
   $nm_saida->saida("   {\r\n");
   $nm_saida->saida("       NovaJanela = window.open (\"grid_gerens_doc.php?nmgp_parms=\" + campo1, \"ScriptCase\", \"resizable\");\r\n");
   $nm_saida->saida("   }\r\n");
   ...
   {
      global $nm_saida;
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['det_print'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_gerens']['pdf_det']) 
   { 
       ...
       if ($this->nmgp_botoes['det_pdf'] == "on")
       {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Dpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_gerens/grid_gerens_config_pdf.php?nm_opc=pdf_det&nm_target=0&nm_cor=cor&papel=1&orientacao=1&largura=1200&conf_larg=S&conf_fonte=10&language=es&conf_socor=N&sc_ver_93=s&password=n&pdf_zip=N&KeepThis=false&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
       if ($this->nmgp_botoes['det_print'] == "on")
       {
         $this->nm_btn_exist['det_print'][] = "Dprint_top";
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Dprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "grid_gerens/grid_gerens_config_print.php?nm_opc=detalhe&nm_cor=CO&password=n&language=es&KeepThis=true&TB_iframe=true&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
       }
