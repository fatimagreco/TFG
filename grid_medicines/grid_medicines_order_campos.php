...
<?php
   ...
   $Ord_Cmp = new grid_medicines_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_medicines_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select_orig'] = array();
   }
   ...
   $tab_def_campos['meds_desc'] = "meds_desc";
   $tab_labels["meds_desc"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['labels']["meds_desc"])) ? $_SESSION['sc_session'][$sc_init]['grid_medicines']['labels']["meds_desc"] : "Medicamento";
   $tab_ger_campos['mtype_id'] = "on";
   $tab_def_campos['mtype_id'] = "mtype_id";
   $tab_labels["mtype_id"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['labels']["mtype_id"])) ? $_SESSION['sc_session'][$sc_init]['grid_medicines']['labels']["mtype_id"] : "Tipo ";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_medicines']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_medicines']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_medicines_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_medicines']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
