<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_vw_anamnesis']['glo_nm_path_imag_temp'] . "/sc_apl_default_EnfermeriaUNAE.txt"))
       {
?>
...
<?php
   ...
   $Sel_Groupby = new grid_vw_anamnesis_sel_Groupby(); 
   $Sel_Groupby->Sel_Groupby_init();
   
class grid_vw_anamnesis_sel_Groupby
{
function Sel_Groupby_init()
{
   ...
   if ($this->restore) {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['all']['SC_Ind_Groupby'];
   }
   else {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Ind_Groupby'];
   }
...
function Sel_processa_out($sel_groupby)
{
   ...
   {
       $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Ind_Groupby']     = $sel_groupby;
       $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['contr_array_resumo'] = "NAO";
       $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['contr_total_geral']  = "NAO";
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['ordem_quebra']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['ordem_select']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['tot_geral']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['Page_break_PDF']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['arr_total']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_group_by']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_x_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_y_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_fill']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_order']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_order_col']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_order_level']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_order_sort']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_order_start']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['pivot_tabular']);
   }
   if ($opc_ret == "resumo" && !empty($this->summ_fields))
   {
       $summ_fields = explode("@?@", $this->summ_fields);
       if(isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'][ $sel_groupby ]))
       {
           foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'][ $sel_groupby ] as $i_sum => $d_sum)
           {
               $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'][ $sel_groupby ][$i_sum]['display'] = in_array($i_sum, $summ_fields);
           }
       }
       $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_order'][ $sel_groupby ] = $summ_fields;
       $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_drill_down'] = $this->drill_down;
   }
?>
    <script language="javascript"> 
<?php
...
?>
 <script language="javascript" type="text/javascript">
  function scSubmitGroupBy(sPos) {
   $.ajax({
    type: "POST",
    url: "grid_vw_anamnesis_sel_groupby.php",
    data: {
...
<?php
if ($this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
...
elseif (!$this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
        if(is_array($_arr_fields))
        {
            foreach ($_arr_fields as $_key_total => $d_field)
...
{
  $max_height = 1;
  if ($this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
  ...
  elseif (!$this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
      if (count($_i_sum) > $max_height)
      {
          $max_height = count($_i_sum);
...
<?php
if ($bSummaryPage && $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Ind_Groupby'] == "sc_free_group_by") {
?>
   if (fieldList == "") {
...
<?php
    ...
    $iTabCount            = 0;
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_All_Groupby'] as $QB => $Tp)
    {
        if (!in_array($QB, $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Groupby_hide']))
        {
...
<?php
     if (!in_array("anam_date_user_docnum_pati_career_pati_caryerar", $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Groupby_hide']))
     {
         $this->static_gb_ok = "sc_id_sel_groupby_anam_date_user_docnum_pati_career_pati_caryerar";
        if ($groupby_atual == "anam_date_user_docnum_pati_career_pati_caryerar")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_anam_date_user_docnum_pati_career_pati_caryerar";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_anam_date_user_docnum_pati_career_pati_caryerar");display_btn_restore_gb();' name="sel_groupby" value="anam_date_user_docnum_pati_career_pati_caryerar" id="sc_id_sel_groupby_anam_date_user_docnum_pati_career_pati_caryerar"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_anam_date_user_docnum_pati_career_pati_caryerar">
        Años
<?php
    $prep_labels = array();
...
?>
<?php
     if (!in_array("user_id", $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Groupby_hide']))
     {
        if ($groupby_atual == "user_id")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_user_id";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_user_id");display_btn_restore_gb();' name="sel_groupby" value="user_id" id="sc_id_sel_groupby_user_id"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_user_id">
        Usuario
<?php
    $prep_labels = array();
    $prep_labels[] = "Usuario";
...
?>
<?php
     if (!in_array("anam_caryear_anam_carsem", $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Groupby_hide']))
     {
        if ($groupby_atual == "anam_caryear_anam_carsem")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_anam_caryear_anam_carsem";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_anam_caryear_anam_carsem");display_btn_restore_gb();' name="sel_groupby" value="anam_caryear_anam_carsem" id="sc_id_sel_groupby_anam_caryear_anam_carsem"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_anam_caryear_anam_carsem">
        Carrera Curso
<?php
    $prep_labels = array();
    $prep_labels[] = "Carrera";
    $prep_labels[] = "Semestre";
?>
        <span>
        <?php echo implode(', ', $prep_labels);?>
        </span>
...
<?php
   ...
   if ($bSummaryPage)
   {
       ...
       if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_control'])) {
           $arr_filds = array();
       }
       else {
           $arr_filds = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_control'] : $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_control'];
       }
    if (isset($arr_filds))
    {
?>
...
<?php
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'])) {
              $arr_disp = array();
          }
          else {
                 $arr_disp = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_display'] : $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_display'];
          }
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_order'])) {
              $arr_ord = array();
          }
          else {
              $arr_ord  = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['res']['summarizing_fields_order']   : $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['summarizing_fields_order'];
          }
...
<script type="text/javascript">
   ...
   if ($this->restore) {
       $oJson = new Services_JSON();
        ob_end_clean();
        if ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['all']['SC_Ind_Groupby'] == "sc_free_group_by") {
           $this->Arr_result['run_js'][] = "scShowFree()";
        } else {
           $this->Arr_result['set_var'][] = array('var' => 'actual_static_gb', 'value' => $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb']['all']['SC_Ind_Groupby']);
...
<?php
   $disp_rest = "none";
   foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['dados_orig_gb'] as $tp => $fields) {
       foreach ($fields as $name => $data) {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis'][$name])) {
               if ($tp == "all" && ($name != "SC_Gb_Free_cmp" || $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis']['SC_Ind_Groupby'] == "sc_free_group_by")) {
                   if ($data != $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis'][$name]) {
                       $disp_rest = "";
                       break;
                   }
               }
               elseif ($tp == "res" && $bSummaryPage && $data != $_SESSION['sc_session'][$sc_init]['grid_vw_anamnesis'][$name]) {
                   $disp_rest = "";
                   break;
               }
           }
