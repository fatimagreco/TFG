...
function scFocusField(sField) {
  ...
  if ($oField.length > 0) {
    switch ($oField[0].name) {
      ...
      case 'pos_id':
      case 'gerens_id':
        sc_exib_ocult_pag('form_Users_mob_form0');
        break;
      case 'f_useres':
        sc_exib_ocult_pag('form_Users_mob_form1');
        break;
    }
  }
...
function displayChange_field_f_useres(row, status) {
    var fieldId;
	if ("on" == status && typeof $("#nmsc_iframe_liga_sec_form_edit_users_mob")[0].contentWindow.scRecreateSelect2 === "function") {
		$("#nmsc_iframe_liga_sec_form_edit_users_mob")[0].contentWindow.scRecreateSelect2();
	}
}
...
function scHidePage(pageNo) {
	$("#id_form_Users_mob_form" + pageNo).hide();
}
...
function scJQCalendarAdd(iSeqRow) {
  $("#id_sc_field_user_bod" + iSeqRow).datepicker({
    ...
    onClose: function(dateText, inst) {
      do_ajax_form_Users_mob_validate_user_bod(iSeqRow);
    },
    showWeek: true,
    numberOfMonths: 1,
    changeMonth: true,
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'form_Users_mob')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
