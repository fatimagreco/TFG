...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_Users']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_Users']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_Users"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_Users']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_Users_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_Users_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
    document.getElementById('form_Users_form0').style.width='1px';
    document.getElementById('form_Users_form0').style.height='0px';
    document.getElementById('form_Users_form0').style.display='none';
    document.getElementById('form_Users_form0').style.overflow='scroll';
    document.getElementById('form_Users_form1').style.width='1px';
    document.getElementById('form_Users_form1').style.height='0px';
    document.getElementById('form_Users_form1').style.display='none';
    document.getElementById('form_Users_form1').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_Users']['pdf_view']) {
?>
...
?>
    ...
    displayChange_page(pag_ativa.substr(15), 'off');
    pag_ativa = N_pagina;
    sc_tab_pag(pag_ativa, 'scTabActive');
    displayChange_page(pag_ativa.substr(15), 'on');
   if ("form_Users_form1" == N_pagina) {
